...
local function log(message)
    ...
    -- Выводим только английские логи в консоль
    -- Кириллица в CC:Tweaked консоли не работает
    print(logEntry)
...
local function handleChatMessage(player, message)
    if player ~= PLAYER_NAME then
        ...
        chatBox.sendMessage("Access denied.", player)
        return
    end
    ...
    if not aiResponse then
        ...
        chatBox.sendMessage("Sorry, technical error occurred. Try later.", player)
...
local function main()
    ...
    log("System ready")
    log("Authorized player: " .. PLAYER_NAME)
