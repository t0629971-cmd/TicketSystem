...
local RELAY_NAME = "redstone_relay_0"  -- Konkretnoye imya releya
local RELAY_OUTPUT_SIDE = "back"  -- Storona releya dlya vykhoda signala (izmeni esli nuzhno)
...
local function initializePeripherals()
    ...
    log("Redstone Relay ready: " .. RELAY_NAME)
end
...
local function openDoor()
    ...
    local relay = peripheral.wrap(RELAY_NAME)
    if relay then
        relay.setOutput(RELAY_OUTPUT_SIDE, true)
        sleep(DOOR_OPEN_TIME)
        relay.setOutput(RELAY_OUTPUT_SIDE, false)
        log("Door closed (via relay " .. RELAY_NAME .. " side " .. RELAY_OUTPUT_SIDE .. ")")
    else
        log("ERROR: Cannot find relay " .. RELAY_NAME)
    end
end
